...
class SimpleVendingMachineGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("智能饮料贩卖机系统 - 简化版")
    ...
    def create_widgets(self):
        ...
        # 左侧控制面板
        control_frame = tk.Frame(main_frame, bg='white', relief='raised', bd=2)
        control_frame.pack(side='left', fill='y', padx=(0, 5))
        
        # 模型设置
        model_frame = tk.LabelFrame(control_frame, text="模型设置", font=('Arial', 10, 'bold'), 
        ...
        tk.Entry(model_path_frame, textvariable=self.model_path_var, 
                state='readonly', width=25).pack(side='left', fill='x', expand=True)
        
        tk.Button(model_path_frame, text="选择", command=self.select_model, 
                 bg='#3498db', fg='white', relief='flat').pack(side='right', padx=(5, 0))
        
        tk.Button(model_frame, text="加载模型", command=self.load_model, 
                 bg='#27ae60', fg='white', relief='flat', padx=20, pady=5).pack(fill='x', pady=5)
        
        # 图片选择
        image_frame = tk.LabelFrame(control_frame, text="图片选择", font=('Arial', 10, 'bold'), 
                                   bg='white', padx=10, pady=10)
        ...
        tk.Entry(before_frame, textvariable=self.before_path_var, state='readonly', width=20).pack(side='left', fill='x', expand=True)
        ...
        tk.Entry(after_frame, textvariable=self.after_path_var, state='readonly', width=20).pack(side='left', fill='x', expand=True)
        ...
        # 结果显示区域
        result_label_frame = tk.LabelFrame(result_frame, text="分析结果", font=('Arial', 10, 'bold'), 
                                          bg='white', padx=10, pady=10)
        result_label_frame.pack(fill='both', expand=True, padx=10, pady=5)
        
        self.result_text = scrolledtext.ScrolledText(result_label_frame, font=('Consolas', 9))
    ...
    def load_default_model(self):
        ...
        for path in default_paths:
            if os.path.exists(path):
                ...
                self.status_var.set(f"找到默认模型: {os.path.basename(path)}")
                break
...
def main():
    """主函数"""
    root = tk.Tk()
    app = SimpleVendingMachineGUI(root)
    root.mainloop()
